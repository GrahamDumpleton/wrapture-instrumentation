"""Drive requests against a local server with the instrumentation
applied.
...
redirect (one leaf, the nested send hidden beneath it), and a 404
(a status like any other: requests raises nothing for it). Each
...
def add_otel_sink() -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            " demo through `just demo-requests --otel`, which overlays"
            " wrapture[otel] for the run"
        ) from error

    wrapture.add_sink(wrapture.otel.sink(service_name="wrapture-requests-demo"))
...
def main(arguments: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="demo.external_requests",
        description="Drive requests against a local server with the"
    ...
    with wrapture.instrumentation("requests"), wrapture.timeline() as tape:
        import requests
    ...
    if options.otel:
        ...
        print()
        print("== otel ==")
        print(f"spans flushed to {endpoint} as service wrapture-requests-demo")
