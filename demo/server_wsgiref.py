"""Serve applications through an instrumented wsgiref.simple_server,
driven by an instrumented urllib client, in one process.
...
- A Flask application, with the flask instrumentation applied as
  well: the framework's own recording middleware sits inside the one
  the server interposes, and each request still records as one tree,
...
def add_otel_sink() -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            " demo through `just demo-wsgiref --otel`, which overlays"
            " wrapture[otel] for the run"
        ) from error

    wrapture.add_sink(wrapture.otel.sink(service_name="wrapture-wsgiref-demo"))
...
def main(arguments: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="demo.server_wsgiref",
        description="Serve applications through an instrumented"
        " wsgiref.simple_server, driven by an instrumented urllib client,"
    ...
    with (
        wrapture.instrumentation("wsgiref.simple_server"),
    ...
    if options.otel:
        ...
        print()
        print("== otel ==")
        print("spans flushed to", endpoint, "as service wrapture-wsgiref-demo")
