...
and a Flask application under both instrumentations."""
...
def test_a_flask_application_records_once_with_its_route(tape: Tape) -> None:
    # The flagship composition: Flask's instrumentation wraps the
    # application's own wsgi_app, the wsgiref instrumentation wraps
    # whatever the server serves, and a request still records as one
    # tree, the framework's annotations landing on the one boundary.
    ...
    with (
        ...
        instrumentation(WSGIRefSimpleServerInstrumentation),
    ):
