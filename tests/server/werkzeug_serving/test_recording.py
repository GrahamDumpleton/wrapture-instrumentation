...
ignore_paths filter, and the trace join."""
...
def test_ignore_paths_records_nothing_at_all(tape: Tape) -> None:
    with instrumentation(WerkzeugServingInstrumentation, ignore_paths=["/health"]):
        serving = serve(hello_app)
        url = next(serving)
        try:
            fetch(f"{url}/health")
...
def test_redact_masks_named_parameters(tape: Tape) -> None:
    with instrumentation(WerkzeugServingInstrumentation, redact=["voucher"]):
        serving = serve(hello_app)
        url = next(serving)
        try:
