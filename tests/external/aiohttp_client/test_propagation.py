...
def test_every_redirect_hop_carries_the_identity(server: Server, tape: Tape) -> None:
    run(lambda s: get(s, f"{server.url}/redirect"))
    ...
    # aiohttp copies the request's headers onto each redirect hop, so
    # the identity travels as the very same header rather than a fresh
    # one per hop.
...
def test_an_applications_own_header_is_left_alone(server: Server, tape: Tape) -> None:
    ...
    run(lambda s: get(s, f"{server.url}/ok", headers={"traceparent": own}))
...
def test_propagate_off_sends_no_trace_headers(server: Server) -> None:
    with (
        instrumentation(AiohttpClientInstrumentation, propagate=False),
        timeline() as tape,
    ):
        run(lambda s: get(s, f"{server.url}/ok"))
...
def test_nothing_recording_means_nothing_to_propagate(server: Server) -> None:
    ...
    with instrumentation(AiohttpClientInstrumentation):
        run(lambda s: get(s, f"{server.url}/ok"))
...
def test_no_identity_is_sent_beneath_a_foreign_leaf(server: Server, tape: Tape) -> None:
    ...
    async def vendor_call(session: ClientSession) -> None:
        await get(session, f"{server.url}/ok")
