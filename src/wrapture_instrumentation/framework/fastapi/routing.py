...
- APIRoute.__init__ substitutes wrapture.observed(endpoint) as
  ...
  The `views` aspect switches this binding and supplies its recording
  options: by default the endpoint's arguments record as their types
  (a parsed body arrives as a model, which any richer level would
  print in full) and its result as its shape (a dict or list is the
  response body), and a `[instrument.views]` table changes either.
...
def observing_endpoints(options: Mapping[str, Any]) -> Callable[..., Any]:
    ...
    options, into an APIRoute construction.
