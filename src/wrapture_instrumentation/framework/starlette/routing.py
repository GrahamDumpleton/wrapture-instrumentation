...
- Route.handle runs once routing has matched, which makes it the
  ...
  it owns, the aspect below the mount point. wrapture's OpenTelemetry
...
- Route.__init__ substitutes wrapture.observed(endpoint) as routes
  ...
  so routes are built afterwards. The `views` aspect switches this
  binding and supplies its recording options: by default the
  endpoint's arguments record as their types (a parsed body arrives
  as a model, which any richer level would print in full) and its
  result as its shape (a dict or list is the response body), and a
  `[instrument.views]` table changes either.
...
def observing_endpoints(options: Mapping[str, Any]) -> Callable[..., Any]:
    """Build the transform that substitutes an observed endpoint,
    labelled by the route's name and recording with the views aspect's
    options, into a Route construction.
    ...
    def transform(
        ...
        observed = wrapture.observed(endpoint, label=name, **options)
...
def instrument(module: Any, instrumentation: wrapture.Instrumentation) -> None:
    ...
    named: dict[str, wrapture.Binding] = {"dispatch": dispatch}

    # The views aspect gates the endpoint observation; the route
    # annotation is core and binds either way.

    views = instrumentation.settings["views"]

    if views.enabled:
        constructor = wrapture.binding(module.Route, "__init__", when=False)
        constructor.on_call.transforms_args(observing_endpoints(views.options))
