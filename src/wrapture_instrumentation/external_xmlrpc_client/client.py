...
carries) plus the RPC method name as `operation`, which is what the
path cannot say. The private name is the honest choke point, and the
recorded path says exactly where the patch lives.
...
def describe(module: Any, proxy: Any, methodname: str) -> dict[str, Any]:
    ...
    sent, plus the RPC method name as operation."""
