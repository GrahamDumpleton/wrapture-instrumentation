...
| [`urllib`](https://github.com/GrahamDumpleton/wrapture-instrumentation/blob/develop/src/wrapture_instrumentation/external_urllib/README.md) | Standard library | Every request through `urllib.request` as one external leaf carrying method, URL, host, port, path and status; the trace identity propagated in its headers; the query string, body and response kept out of capture. | `leaf`, `propagate` |
